...
function initUI() {
	...
	$(".obj-label-enable").click(function(){
		var parent = $(this).parents('.switch');
		$('.obj-label-disable',parent).removeClass('selected');
		$(this).addClass('selected');
		g.drawObjLabelsFlag = 1;
	});
	$(".obj-label-disable").click(function(){
		var parent = $(this).parents('.switch');
		$('.obj-label-enable',parent).removeClass('selected');
		$(this).addClass('selected');
		g.drawObjLabelsFlag = 0;
	});

	$(".flow-label-enable").click(function(){
		var parent = $(this).parents('.switch');
		$('.flow-label-disable',parent).removeClass('selected');
		$(this).addClass('selected');
		g.drawFlowLabelsFlag = 1;
	});
	$(".flow-label-disable").click(function(){
		var parent = $(this).parents('.switch');
		$('.flow-label-enable',parent).removeClass('selected');
		$(this).addClass('selected');
		g.drawFlowLabelsFlag = 0;
	});

	$('#perspective-disable').addClass('selected');
	$('#perspective-enable').removeClass('selected');
	$('#obj-label-disable').addClass('selected');
	$('#obj-label-enable').removeClass('selected');
	$('#flow-label-disable').removeClass('selected');
	$('#flow-label-enable').addClass('selected');
	togglePerspective(0);
}
...
function setParam(event, ui, obj, name) {
	...
	if (name == 'Scale') {
		...
		g.scale = value;
	} else if (name == 'Direction of Switch') {
		$('#switch_rotation_slider').text($.sprintf(": % 4d", value));
		g.switch_rotate_param = value;
	} else if (name == 'Direction of Controller') {
		$('#controller_rotation_slider').text($.sprintf(": % 4d", value));
		g.controller_rotate_param = value;
	} else if (name == 'Direction of Host') {
		$('#host_rotation_slider').text($.sprintf(": % 4d", value));
		g.host_rotate_param = value;
...
function setupSlider($, elem, ui, obj) {
	...
	if (ui.name == 'Scale') {
		...
		g.scale = ui.value;
	} else if (ui.name == 'Direction of Switch') {
		$('#switch_rotation_slider').text($.sprintf(": % 4d", ui.value));
		g.switch_rotate_param = ui.value;
	} else if (ui.name == 'Direction of Controller') {
		$('#controller_rotation_slider').text($.sprintf(": % 4d", ui.value));
		g.controller_rotate_param = ui.value;
	} else if (ui.name == 'Direction of Host') {
		$('#host_rotation_slider').text($.sprintf(": % 4d", ui.value));
		g.host_rotate_param = ui.value;
